#!/usr/bin/env python3
"""
Decode_Sound full experiment for PsychoPy 2024.2.4.

Run this file inside PsychoPy (Coder) or from a PsychoPy-enabled Python
environment. This script implements the legacy full experiment structure.
"""
...
def wait_for_mouse_press(mouse: event.Mouse, clock: core.Clock) -> float:
    mouse.clickReset()
    while True:
        if "escape" in event.getKeys(["escape"]):
            raise KeyboardInterrupt
            ...
            for trial_index in range(1, TRIALS_PER_BLOCK + 1):
                ...
                while not valid_trial:
                    ...
                    if block_type == "active":
                        ...
                        button_down = wait_for_mouse_press(mouse, clock)
